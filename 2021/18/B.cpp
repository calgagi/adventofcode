...
int main() {
    ...
    auto copy = cp::rlamb([&](auto&& copy, node* c) -> node* {
        ...
        if (c->right != nullptr) {
            cc->right = copy(c->right);
            cc->right->par = cc;
        }
        return cc;
    });

    auto del = cp::rlamb([&](auto&& del, node* c) -> void {
        if (c == nullptr) return;
        del(c->left);
        del(c->right);
        delete c;
    ...
    for (int i = 0; i < (int) nums.size(); i++) {
        for (int j = i + 1; j < (int) nums.size(); j++) {
            ...
            head->left = copy(nums[i]);
            head->left->par = head;
            head->right = copy(nums[j]);
            head->right->par = head;
            testcase(head);
            ans = max(ans, magnitude(head));
            del(head->left), del(head->right);

            head->left = copy(nums[j]);
            head->left->par = head;
            head->right = copy(nums[i]);
            head->right->par = head;
            testcase(head);
            ans = max(ans, magnitude(head));
            del(head->left), del(head->right);
